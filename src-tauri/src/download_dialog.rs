...
pub fn handle_service_webview_download<R: Runtime>(
    ...
    match event {
        DownloadEvent::Requested { url, destination } => {
            ...
            let start_dir = destination.parent().map(Path::to_path_buf).or(downloads);

            let window = webview.window();
            let mut dialog = FileDialog::new().set_title("Save file").set_parent(&window);
        ...
        DownloadEvent::Finished { success, .. } => {
            if !success {
                let _ = webview.app_handle().emit("show-toast", "Download failed.");
            }
            true
        }
        _ => true,
